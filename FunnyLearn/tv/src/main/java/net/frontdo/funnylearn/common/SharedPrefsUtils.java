...
public class SharedPrefsUtils {
    // userInfo
    public static final String SP_FILE_NAME_USER = "frondto_user_info";         // 用户信息文件名
    ...
    private <T> T getValue(String key, Class<T> clazz, SharedPreferences sp) {
        ...
        try {
            ...
            e.printStackTrace();
            FrontdoLogger.getLogger().e("system", "类型输入错误或者复杂类型无法解析[" + e.getMessage() + "]");
        } catch (IllegalAccessException e) {
            e.printStackTrace();
            FrontdoLogger.getLogger().e("system", "类型输入错误或者复杂类型无法解析[" + e.getMessage() + "]");
        }
        FrontdoLogger.getLogger().e("system", "无法找到" + key + "对应的值");
        return null;
    }
